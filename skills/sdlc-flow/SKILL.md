...
1. **听清意图**，归入三类：
   ...
   - **硬前置门禁**（硬）：「某阶段前必须做某步」→ 写 `phasePreconditions[<phase>]`，每项 `{ step, requireArtifact, reason }`。`requireArtifact` 是该步必须产出的文件（相对任务目录）；hooks 据其存在与否在该阶段的源码编辑处**硬拦**。
...
```json
{
  ...
  "phasePreconditions": {
    "implement": [
      { "step": "locate-code", "requireArtifact": "onlyAI/locate-code.md", "reason": "codegraph 检索待修改部分" }
    ]
  }
}
```

工具「选哪个」是软推荐（codegraph 优先、可降级）；「这一步必须发生」是硬门禁——缺 `onlyAI/locate-code.md` 时 implement 的源码编辑被 block。这正是「流程序列软、声明的必做动作硬」。
